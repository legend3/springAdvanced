...
1.准备(工厂)bean。实现类和重写方法  2.注册bean(myFactory类)。注册到@Bean中  
(创建Bean对象时)注意：需要通过&区分 获取的对象是哪一个 ： 不加&,获取的是最内部真实的Apple；  
...
1. 如果是注解形式 ， 随便写一个方法 ，然后加上相应注解即可  
2. 如果是接口形式，必须 实现接口中**规定的方法**  

#### 方法四：（给容器中的**所有Bean加**初始化、销毁）一个接口   (案例: MyXxx类)  
接口：适用于三层组件  
